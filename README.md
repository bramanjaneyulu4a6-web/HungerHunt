...
The 458 backend tests cover the parent API surface and auth. The frontends are lightly covered — `frontend-admin`, `hungerhunt-kiosk` and `hungerhunt-warehouse` each have a suite that CI runs, mostly over utilities and the shared availability rule, plus one hook and one component in the kiosk; `frontend-parent` has none. Whole screens and flows are still verified by hand.
