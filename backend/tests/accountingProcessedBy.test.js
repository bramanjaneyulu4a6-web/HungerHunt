...
describe('narrowing the collections to those people', () => {
  ...
  test('staff alone reads their deposits and refunds, and no charge', () => {
...
describe('the routes', () => {
  ...
  test('a staff filter on the CSV reads no charges and only their deposits', async () => {
    mock.method(Admin, 'exists', async () => ({ _id: ADMIN_ID }));
    ...
    assert.equal(response.status, 200);
    assert.equal(charges.mock.callCount(), 0);
