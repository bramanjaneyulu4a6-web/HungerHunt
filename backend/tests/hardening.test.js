...
const assertOnlyIdentity = (written, label) => {
  const expected = { ...IDENTITY, roomId: ROOM_ID };
  assert.deepEqual(
...
const stubLinking = () => {
  ...
  mock.method(Room, 'findOne', () => ({
    lean: async () => ({ _id: ROOM_ID, code: IDENTITY.roomNumber }),
  }));
...
describe('only identity and resolved room fields are writable', () => {
  before(() => {});
