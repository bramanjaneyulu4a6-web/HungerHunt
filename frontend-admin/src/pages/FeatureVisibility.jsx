...
/* School-wide rules on who may order. Not visibility settings — the server
   enforces them — but they are the super admin's alone, so they live on the
   super admin's page. */
...
const RULE_TOASTS = {
  requireActivatedParent: {
    on: 'Students now need an activated parent to order',
    off: 'Students can order without an activated parent',
  },
  oneOrderPerWeek: {
...
function OrderingRulesCard() {
  ...
  const toggle = async (field) => {
    ...
    setSaving(true);
    try {
      const response = await api.put('/admin/settings/ordering', { [field]: next });
      setSettings(response.data);
      toast.success(RULE_TOASTS[field][next ? 'on' : 'off']);
    } catch (saveError) {
    ...
    <Card className="feature-card">
      <div className="users-tab-head">
        <div>
          ...
          <p>
            <strong>Require an activated parent:</strong> only allow a student to order at the kiosk
            once a parent has activated their account (signed in to the parent app and set a
            password). Students whose parent has not are turned away at the kiosk with a message
            asking the parent to activate.
          </p>
          <p>
            <strong>One order per week:</strong> a student may place one order each week, starting
        ...
        {settings && (
          <div>
            {ORDERING_RULES.map(({ field, label }) => (
              <label key={field} className="student-picker__row">
                <input
                  type="checkbox"
                ...
                />
                <span>{label}</span>
                <Badge variant={settings[field] ? 'success' : 'neutral'}>
